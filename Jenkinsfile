        ...
        stage('Compute Image Tag') {
            ...
            steps {
                script {
                    ...
                    env.IMAGE_TAG = params.IMAGE_TAG ?: "v${env.BUILD_NUMBER}-${gitCommit}"
                }
            }
        }
            ...
            steps {
                script {
                    ...
                    try {
                        ...
                        def response = sh(script: "curl -s -o /dev/null -w '%{http_code}' http://localhost:${testPort == 0 ? '80' : testPort}/", returnStdout: true).trim()
                        if (response != '200') { error "Web application did not respond correctly. HTTP ${response}" }
                ...
                dir('TerraformDep') {
                    script {
                        withAWS(credentials: 'access-key', region: "${params.AWS_REGION}") {
                            sh '''
#!/bin/bash
terraform init || { echo "Terraform init failed. Skipping apply."; exit 0; }
...
terraform destroy -target=kubernetes_deployment.app -auto-approve || true
terraform destroy -target=kubernetes_service_account.load_balancer_controller -auto-approve || true
...
docker ps -q --filter "name=test-container-${BUILD_NUMBER}" | xargs --no-run-if-empty docker stop || true
docker ps -a -q --filter "name=test-container-${BUILD_NUMBER}" | xargs --no-run-if-empty docker rm || true
'''
                }
